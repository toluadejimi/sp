@if ($basic_settings->kyc_verification == true && isset($user_kyc) && $user_kyc != null && $user_kyc->fields != null)
    <h3 class="title">{{ __("KYC Information") }} &nbsp;
        <span class="{{ auth()->user()->kycStringStatus->class }}">{{__( auth()->user()->kycStringStatus->value) }}</span>
    </h3>
    ...
    @else
        ...
        <form action="{{ setRoute('user.authorize.updateinfo.submit') }}" class="account-form" method="POST" enctype="multipart/form-data">
            @csrf
...
{{--                        @endif--}}
    ...
    <form action="{{ setRoute('user.authorize.verify.submit') }}" class="account-form" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row ml-b-20">

            <div class="col-lg-12 form-group text-center">
                <label class="d-flex justify-content-start">BVN</label>
                <input class="form-control text-dark" name="bvn" value="{{Auth::user()->bvn}}">
            </div>

            <div class="col-lg-12 form-group text-center">
                <label class="d-flex justify-content-start">NIN</label>
                <input class="form-control text-dark" name="nin" value="{{Auth::user()->nin}}">
            </div>

            <div class="col-lg-12 form-group text-center">
                <button type="submit" class="btn--base w-100">{{ __("Verify Now") }}</button>
            </div>
        </div>
