            ...
            <div class="row justify-content-center">
                <div class="col-lg-6 col-md-8 pb-30">
                    <div class="deposit-form">
                        ...
                        <div class="row justify-content-center">
                            <form class="card-form" action="{{ setRoute("user.transfer.money.vendorsend") }}" method="POST">
                             @csrf
                            <div class="col-lg-12">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" required class="form--control checkUser" placeholder="Enter Vendor ID" name="email2" value="{{ old("vendor_id") }}">
                                    <label class="exist2 text-start my-3"></label>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <label class="username text-start my-3 text-bold"></label>
                                </div>
...
<script>
      $('.checkUser').on('keyup',function(e){
            var url = '{{ route('user.transfer.money.vendor.exist') }}';
            var value = $(this).val();
            var token = '{{ csrf_token() }}';
            if ($(this).attr('name') == 'email2') {
                var data = {email2:value,_token:token}

            }
            $.post(url,data,function(response) {
                if(response.own){
                    if($('.exist2').hasClass('text--success')){
                        $('.exist2').removeClass('text--success');
                    }
                    $('.exis2t').addClass('text--danger').text(response.own);
                    $('.transferBtn2').attr('disabled',true)
                    return false
                }
                if(response['data'] != null){
                    if($('.exist2').hasClass('text--danger')){
                        $('.exist2').removeClass('text--danger');
                    }
                    let success = response['data'].message;
                    if(success === "No merchant found"){
                        $('.exist2').text(success).addClass('text--danger');
                        $('.transferBtn2').attr('disabled',true)
                    }else{

                        let vendor = response['data'].data;
                        $('.exist2').text(vendor).addClass('text--success');
                    ...
                    $('.exist2').text('User doesn\'t  exists.').addClass('text--danger');
                    $('.transferBtn2').attr('disabled',true)
                    return false
                }
        ...
        async function sendPostRequest() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    usernameLabel.textContent = username || "No username found.";
                    usernameLabel.style.color = "green";


                } else {
                    usernameLabel.textContent = result.error || "An error occurred.";
                    usernameLabel.style.color = "red";
                }
            } catch (error) {
                usernameLabel.textContent = "Request failed. Please try again.";
                usernameLabel.style.color = "red";
                console.error("Error:", error);
            }
        }

        // Add event listener to trigger on input change in email
        emailInput.addEventListener("blur", sendPostRequest);
    });
</script>
