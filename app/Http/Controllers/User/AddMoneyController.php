...
class AddMoneyController extends Controller
{
    ...
    public function index() {
        ...
        if($ck_va == null){
            return back()->with(['error' => [__('You need to create a Virtual Account')]]);
        }
    ...
    public function submit(Request $request) {
        ...
        $key = env('WEBKEY');
        $ref = "VCARD".random_int(0000000000,9999999999);
        $email = Auth::user()->email;
        $amount = $request->amount;
        $url = "https://web.enkpay.com/pay?amount=$amount&key=$key&ref=$ref&email=$email";


        $trx = new Transaction();
        $trx->user_id = Auth::id();
        $trx->trx_id = $ref;
        $trx->request_amount = $amount;
        $trx->remark = "Instant FUnding";
        $trx->status = 2;
        $trx->type = "Wallet Instant Fund";
        $trx->save();
