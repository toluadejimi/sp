...
class AddMoneyController extends Controller
{
    ...
    public function webhook(Request $request)
    {
        try {
            ...
            $acct = VirtualAccount::where('account_no', $acc_no)->first()->user_id ?? null;
            ...
            $status = Transaction::where('session_id', $session_id)->first()->status ?? null;
            if ($status == 1) {
                return response()->json([
                    'status' => false,
            ...
            $trx->request_amount = $p_amount;
            $trx->payable = $p_amount;
            $trx->available_balance = $available_balance;
            $trx->status = 1;
            $trx->attribute = 'RECEIVED';
